...
async def get_level_id_word(level_name: str) -> int | None:
    """
    Возвращает lev_id из view_study_level_mapped по level_word.
    Например, "средний" → 2
    """
    ...
    try:
        ...
        row = await conn.fetchrow(
            "SELECT lev_id FROM view_study_level_mapped WHERE LOWER(level_word) = $1",
            level_name.lower()
        )
        await conn.close()
        return row["lev_id"] if row else None
