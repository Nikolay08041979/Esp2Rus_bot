...
#         total_words = words_row["total"]
#
#         # 🎯 Уровень клиента и его lev_id (группа)
#         if level_id:
#             row = await conn.fetchrow(
#                 "SELECT cefr_id_short, lev_id FROM level_matrix WHERE level_id_client = $1", level_id
#             )
#             if row:
...
#             lev_id = 1
#
#         # ✅ Получаем прогресс по уровню из client_level_progress
#         row = await conn.fetchrow("""
#             SELECT coverage_percent FROM client_level_progress
#             WHERE client_id = $1 AND lev_id = $2
#         """, client_id, lev_id)
...
#
#
#         # 📈 Прогресс по уровням (из view_user_progress_by_level)
#         category_rows = await conn.fetch("""
#             SELECT cat_name,
...
#         level_text = "\n".join(category_lines) if category_lines else "–"
#
#         # 📊 Прогресс по темам (из view_user_progress_by_theme)
#         category_rows = await conn.fetch("""
#             SELECT cat_name, total_words, learned_words, percent_done
#             FROM view_user_progress_by_theme
#             WHERE client_id = $1
...
#             percent = row["percent_done"]
#             if learned > 0:
#                 category_lines.append(f"•⁠  ⁠{row['cat_name']}: {learned}/{total} ({percent}%)")
#         category_text = "\n".join(category_lines) if category_lines else "–"
#
#         with open(TEMPLATE_PATH, encoding="utf-8") as f:
#             template = f.read()
